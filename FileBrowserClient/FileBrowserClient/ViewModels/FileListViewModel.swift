...
class FileListViewModel: ObservableObject {
    ...
    func sortedFiles(by sortOption: SortOption) -> [FileItem] {
        files.sorted { firstFile, secondFile in
            ...
            case .sizeAsc: return (firstFile.size) < (secondFile.size)
            case .sizeDesc: return (firstFile.size) > (secondFile.size)
    ...
    func getFiles(baseRequest: Request, at path: String, modifySheet: Bool) {
        ...
        preparedRequest.session.dataTask(with: preparedRequest.request) { data, _, _ in
